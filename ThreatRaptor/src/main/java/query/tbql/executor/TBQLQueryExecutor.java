...
public void printResults() {
    ...
    if (tbqlQueryContext.queryTableResults.isEmpty()) {
        System.out.println("No threats detected in the file.");
        return;
    }

    for (String threat : tbqlQueryContext.queryTableResults) {
        System.out.println("\n⚠️ Detected Threat: " + threat);
        System.out.println(tbqlQueryContext.getMitigationDetails(threat));
    }

    if (!tbqlQueryContext.returnCount) {
        System.out.println("\nTotal threats detected: " + tbqlQueryContext.queryTableResults.size());
    } else {
        System.out.println("\nCount Mode: Threat count returned instead of details.");
    }
